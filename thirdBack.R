...
server <- function(input, output){
  ...
  observeEvent(input$load, {
    ...
    #tryCatch(dataPull(input$ticker),error=function(e){return()}) 
    ...
    tryCatch(dataPull ,error=function(e){return()})
    
    
    
    tickers = spl(paste(toupper(input$ticker))) 
    tickers.temp = spl(paste0('NASDAQ:',toupper(input$ticker)))
